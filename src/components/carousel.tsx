...
const slides = [
  {
    src: "/model-1.png",
    alt: "Look 01 — Masculino Oversized Black",
    label: "Look 01",
    desc: "Oversized T-Shirt Black + Cargo Pants",
    tag: "Masculino",
  },
  {
    src: "/model-2.png",
    alt: "Look 02 — Feminino Forest Green",
    label: "Look 02",
    desc: "Oversized T-Shirt Forest Green + Biker Short",
    tag: "Feminino",
  },
  {
    src: "/model-3.png",
    alt: "Look 03 — Feminino Green",
    label: "Look 03",
    desc: "Oversized T-Shirt Green + Tênis Branco",
    tag: "Feminino",
  },
  {
    src: "/model-4.png",
    ...
    <div className="relative w-full overflow-hidden" style={{ background: "#0a0a0a" }}>
      ...
      <div className="relative">
        ...
        <div className="flex transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${current * 100}%)` }}>
          {slides.map((slide, i) => (
            <div key={i} className="relative flex-shrink-0 w-full" style={{ aspectRatio: "16/9" }}>
              <Image
                src={slide.src}
                alt={slide.alt}
                fill
        ...
        <button
          onClick={prev}
          className="absolute left-4 top-1/2 -translate-y-1/2 flex items-center justify-center w-10 h-10 border border-white/20 bg-black/50 hover:border-white/60 transition-all"
          aria-label="Anterior"
        >
        ...
        <button
          onClick={next}
          className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center justify-center w-10 h-10 border border-white/20 bg-black/50 hover:border-white/60 transition-all"
          aria-label="Próximo"
        >
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.5">
      ...
      <div className="hidden md:flex border-t" style={{ borderColor: "rgba(255,255,255,0.08)" }}>
        {slides.map((slide, i) => (
          ...
          >
            <Image src={slide.src} alt={slide.label} fill className="object-cover object-top" sizes="20vw" />
